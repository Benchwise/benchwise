...
class BenchmarkResult:
    ...
    def get_worst_model(self, metric_name: str = None) -> Optional[EvaluationResult]:
        ...
        return min(successful_results, key=lambda r: r.get_score(metric_name) or float('inf'))
    ...
    def compare_models(self, metric_name: str = None) -> Dict[str, Any]:
        ...
        valid_scores = [(name, score) for name, score in zip(model_names, scores) if score is not None]
        ...
        return {
            'ranking': [{'model': name, 'score': score} for name, score in sorted_results],
...
class ResultsAnalyzer:
    ...
    @staticmethod
    def compare_benchmarks(benchmark_results: List[BenchmarkResult], metric_name: str = None) -> Dict[str, Any]:
        """
        ...
        for benchmark in benchmark_results:
            ...
            for result in benchmark.successful_results:
                ...
                comparison['cross_benchmark_scores'][model_name][benchmark.benchmark_name] = score
    ...
    @staticmethod
    def analyze_model_performance(results: List[EvaluationResult], metric_name: str = None) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def generate_report(benchmark_result: BenchmarkResult, output_format: str = 'text') -> str:
        """
    ...
    def _generate_markdown_report(benchmark_result: BenchmarkResult) -> str:
        ...
        for result in benchmark_result.results:
            ...
            lines.append(f"| {result.model_name} | {status} | {result_str} | {result.duration:.2f}s |")
...
class ResultsCache:
    ...
    def save_result(self, result: EvaluationResult, dataset_hash: str):
        ...
        cache_key = self._get_cache_key(result.model_name, result.test_name, dataset_hash)
        cache_file = self.cache_dir / f"{cache_key}.json"
        
        with open(cache_file, 'w') as f:
            json.dump(result.to_dict(), f, indent=2, default=str)
    
    def load_result(self, model_name: str, test_name: str, dataset_hash: str) -> Optional[EvaluationResult]:
...
def save_results(benchmark_result: BenchmarkResult, file_path: Union[str, Path], format: str = 'json'):
    """
...
def load_results(file_path: Union[str, Path]) -> BenchmarkResult:
    ...
    for result_data in data.get('results', []):
        ...
        result_data_clean = {k: v for k, v in result_data.items() 
                           if k not in ['success', 'failed']}  # These are @property methods
